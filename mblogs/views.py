...
from settings import WEIBO_KEY, WEIBO_SECRET, WEIBO_CALLBACK,TQQ_KEY, TQQ_SECRET
...
def weibo_callback(request):
    ...
    token_url = "https://api.weibo.com/oauth2/access_token?client_id=%s&client_secret=%s&grant_type=authorization_code&redirect_uri=%s&code=%s" % (WEIBO_KEY, WEIBO_SECRET, WEIBO_CALLBACK, code)
    info = JSONDecoder().decode(urlopen(token_url, "").read())
    Token.objects.create(type="sina", token = info)
    return HttpResponse('done.')


def weibo_refresh(request): #simulate the brower to login
    red_url = "https://api.weibo.com/oauth2/authorize?client_id=%s&response_type=code&redirect_uri=%s" % (WEIBO_KEY, WEIBO_CALLBACK)
...
# unused right now
#def weibo_refresh1(request):
    #red_url = "https://api.weibo.com/oauth2/authorize?client_id=%s&response_type=code&redirect_uri=%s" % (WEIBO_KEY, WEIBO_CALLBACK)
    #return redirect(red_url)
    #t = Token.objects.filter(type = 'sina').order_by('-id')[0]
    #ref_url = 'https://api.weibo.com/oauth2/access_token?refresh_token=%s' % (t.token['access_token'])
...
def tqq_auth(request):
    ...
    body = urllib.urlencode(dict(oauth_callback="http://127.0.0.1:8000/tqq_callback"))
